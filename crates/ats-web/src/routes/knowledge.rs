...
async fn status_handler(Extension(state): Extension<Arc<AppState>>) -> Json<KnowledgeStatus> {
...
async fn check_handler(Extension(state): Extension<Arc<AppState>>) -> Json<KnowledgeStatus> {
    let paths = KnowledgePaths::from_runtime_dir(&state.runtime_dir);
    Json(get_status(&paths))
}
