...
本文默认遵循 [2026-04-10-后端双角色与服务器内部执行器边界收口方案.md](./2026-04-10-后端双角色与服务器内部执行器边界收口方案.md) 的上位口径：

- 后端长期只有两个角色：`workstation` 与 `web`
- 服务器侧 `workstation` 是 `workstation` 角色在服务器内部的部署实例
- 服务器侧 `workstation` 不是第三个公开后端角色
- 前端长期只认识两个公开目标：`workstation` 与 `web`

因此，本文聚焦讨论的是：

- 在“双角色”前提下，服务器侧为什么仍然需要保留一个独立部署的 `workstation` 执行器实例
- `web -> 服务器侧 workstation` 的内部协作边界
- 服务器模式的执行拓扑、并行方案与实施顺序

本文不再负责重新定义“后端到底有几个角色”，该问题以上述 2026-04-10 文档为准。
...
本文与 `docs/90-归档/后端专题/已经完成/2026-03-27-后端设计详情.md` 的大方向一致，但比那份文档更具体。
...
当时前端在 `frontend/src/shared/api/http.ts` 中只定义了：
...
- `frontend/src/components/ExecutionModeDialog.tsx`（历史路径）
- `frontend/src/features/platform-run/createAndStartFlow.ts`（历史路径）
- `frontend/src/shared/api/me.ts`（历史路径）
