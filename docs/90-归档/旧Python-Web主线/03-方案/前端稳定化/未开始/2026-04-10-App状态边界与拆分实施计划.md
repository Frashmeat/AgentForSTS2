...
**Goal:** 收口旧 `frontend/src/App.tsx` 的顶层状态边界，把当时的“路由壳 + 多功能总控”大组件拆回可维护的前端结构，同时不破坏旧工作流、用户中心和执行模式切换行为。
...
- 当时的 `frontend/src/App.tsx` 约 808 行。
- 顶层组件同时承担：
  - 路由定义
  - 工作区壳层渲染
...
- [x] 已把 `frontend/src/pages/BatchMode.tsx` 收口为纯壳页，并把批量相关 adoption test 统一改到 `features/batch-generation/view.tsx` 的真实实现入口
- [x] 已把批量 feature 视图中的运行时错误字段与状态模型统一到 `workflowErrorMessage`
- [x] 已把 workspace 路由、返回路径和导航配置抽成共享 `config.ts`，减少 `App.tsx` 与 execution flow 之间的重复 helper
- [x] 已让批量页直接复用 `state.ts` 中的运行时类型与默认状态，减少 `view.tsx` 内部重复模型定义
- [x] 已把批量页的核心运行时状态接回 `batchWorkflowReducer`，减少 `view.tsx` 内部手写 setter 分支
- [x] 已把批量 review 严格度状态接回 `batchWorkflowReducer`，减少 review 流程中的双轨状态
- [x] 已把批量 review 的 `planReview / bundleDecisions` 接回 `batchWorkflowReducer`，让 review 刷新与执行前决策共享同一份运行时状态
- [x] 2026-04-27 已把 `/auth/*`、项目路径选择 / 创建项目、构建 / 打包 / 部署的短暂反馈接入全局 `StatusNoticeStack`；工作流详情错误、页面主体错误和计费确认弹窗暂按原上下文保留。
