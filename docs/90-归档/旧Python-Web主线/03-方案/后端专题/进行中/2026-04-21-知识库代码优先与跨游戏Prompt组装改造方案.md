...
1. `backend/tests/test_sts2_guidance.py`（历史路径）
   - 继续验证摘要资源本身是否存在和内容是否完整
   - 不把它升级成“代码优先链路”测试
2. `backend/tests/test_knowledge_runtime.py`（历史路径）
   - 继续只验证知识资产目录和刷新语义
   - 不与 Prompt 组装逻辑耦合
3. 平台 runner 相关测试
   - 第一阶段不建议让平台执行链同时迁移
