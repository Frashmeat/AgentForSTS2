...
- 当前状态：已经完成。2026-04-29 代码事实显示 `execution_profiles`、`server_credentials`、`credential_health_checks` 与 `user_platform_preferences` 已有迁移、ORM 和仓储实现，本文保留为表结构口径参考。
...
- 第一版 `credential_ref` 不单独新增 schema，固定使用稳定内部引用：
  - `server-credential:{id}`
- 当前这套解析真源已经在后端编排服务内可用，但真实运行时 caller 仍待后续正式接入。
