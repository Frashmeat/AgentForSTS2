...
>
> 归档状态：2026-08-16 作为已完成基线归档；当前状态从
...
Direct Single 仍是单次严格调用。Composition Generate request v6 才启用 Graph-owned semantic feedback、单 Item adjustment 和 Provider-owned pipeline 选择；Shell 只接收有界 View，不接收 feedback envelope、checkpoint 或 raw completion，普通 UI 也不展示内部 round/phase。
...
### ExecutionGraphRecord v4

大型 Plan/Generate 的可恢复状态。Graph 保存 Blueprint identity、nodes、attempt count/active attempt、当前 normalized checkpoint、versioned/hashed safe feedback state、active/previous Runs、graph failure、commit intent、final result ref、graph-total semantic request count 和至多一个 RepairCampaign。一个 graph 最多一个 active Run。v4 只读取 `.ats/execution-graphs-v4`，不迁移或兼容读取 v1/v2/v3；旧目录保留为历史证据。

Composition Generate request v6 把可修订 output-contract diagnostic、registered validator 的多 Item generated-content campaign、版本绑定的单 Item adjustment 和 Provider-owned pipeline 选择纳入同一 Feature-owned controller。模型返回完整 bundle/role set，每轮重新严格 decode，并在 checkpoint 后重建 finalize、重跑完整 validator suite。Graph 总预算不按节点放大，`until_passed` 也有 20 次绝对上限。成功结果调整派生新 Graph/Run/Artifact version并保留源证据；paused pre-commit Graph才允许原地 CAS。`commit_prepared` 只能在 Provider 图要求的 validation 成功后创建；data-only Provider 可以通过受信任 Primitive 明确省略模型、validation、build 和 package。
