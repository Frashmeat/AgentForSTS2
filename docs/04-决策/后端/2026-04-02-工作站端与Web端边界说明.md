# 2026-04-02 工作站端与 Web 端边界说明

> 文档定位：本文固化当前代码实现中的 `workstation-backend` 与 `web-backend` 边界，回答入口、路由、模块、运行依赖和职责归属问题。
>
> 事实依据：基于当前 `backend/app_factory.py`、`backend/routers/__init__.py`、`backend/app/composition/container.py`、`backend/app/shared/infra/config/settings.py` 与平台 runner 代码核实。
>
> 权威入口：上级入口为 `docs/04-决策/后端/README.md`；接口细节见 `docs/02-现状/当前前后端接口文档.md`。
>
> 最后更新：2026-05-04

## 1. 当前结论

当前后端运行角色已经收口为 **`workstation` 与 `web` 两类**：

- `workstation-backend`：本地创作工作站，负责前端静态资源、本地项目工作流、配置、知识库、日志/Mod 分析、构建部署和审批。
- `web-backend`：平台 API 服务，负责认证、当前用户中心、平台任务、配额、执行记录、凭据、审计和后台队列执行。
- `full` 不再是当前代码中的有效运行角色；`backend/main.py` 当前等价于工作站入口，调用 `create_app("workstation")`。

当前部署拓扑进一步收口为一条主线：

- 本机运行 `frontend` 与 `local-workstation`。
- Docker 运行 `web`、`web-workstation` 与 `postgres`。
- 人工配置唯一真实源为 `runtime/agentthespire.config.json`。
- 部署期派生配置统一写入 `runtime/generated/*`，包括三份 role config、前端 `runtime-config.js` 与 Docker env。
- 前端只访问本机 `local-workstation` 与 `web`；`web-workstation` 只接受 Docker 内网中的 `web` 调度。

## 2. 运行角色与入口

| 角色 | 入口文件 | 默认端口 | 前端静态文件 | 路由来源 | 数据库要求 |
| --- | --- | --- | --- | --- | --- |
| `local-workstation` / `workstation` | `backend/main_workstation.py`、`backend/main.py` | `7860` | 本机最终部署由独立 `frontend` 托管；工作站入口仍可挂载 `frontend/dist` 用于直启 | 仅工作站路由 | 默认不要求 |
| `web` | `backend/main_web.py` | `7870` | 否 | 仅 Web 路由 | 要求 `database.url` 与 `auth.session_secret` |
| `web-workstation` / `workstation` | `backend/main_workstation.py` | Docker 内网 `7860` | 否 | 工作站内部执行路由 | 默认不要求数据库；要求与 `web` 共享控制 token |

运行时校验来自 `Settings.validate_for_role(role)`：

- `workstation` 主要校验 CORS 与认证 cookie 名称。
- `web` 额外要求数据库连接与 session secret。

## 3. 边界总图

```mermaid
flowchart TB
    FE["同一个前端壳 / 独立静态前端"]

    subgraph WS["workstation-backend : 7860"]
        WEntry["main.py / main_workstation.py\ncreate_app('workstation')"]
        WRoutes["工作站路由\nworkflow / batch / config / knowledge\nlog / mod / build_deploy / approvals"]
        WStatic["挂载 frontend/dist"]
        WLocal["本地能力\n项目目录 / STS2 / Godot / Mods\n图片处理 / 代码代理 / 构建部署"]
    end

    subgraph WEB["web-backend : 7870"]
        WebEntry["main_web.py\ncreate_app('web')"]
        WebRoutes["Web 路由\nauth / me / platform_jobs / platform_admin"]
        WebDB["数据库真源\nauth / jobs / quota / executions / audit"]
        WebWorker["平台队列 Worker\n执行配置 seed + 后台扫描领取任务"]
    end

FE -->|"工作台 HTTP + WebSocket"| WRoutes
FE -->|"认证 / 用户中心 / 平台任务 HTTP"| WebRoutes
WebRoutes -->|"Docker 内网调度"| WRoutes

    WEntry --> WRoutes
    WEntry --> WStatic
    WRoutes --> WLocal

    WebEntry --> WebRoutes
    WebEntry --> WebDB
    WebEntry --> WebWorker
```

## 4. 路由装配边界

`backend/routers/__init__.py` 定义两个固定路由集合。

### 4.1 工作站路由集合

| 路由模块 | 典型路径 | 作用 | 边界判断 |
| --- | --- | --- | --- |
| `routers.workflow` | `/api/ws/create`、`/api/project/create`、`/api/project/build`、`/api/project/package` | 单资产创建、生图、代码生成、项目创建、构建与打包 | 依赖本地项目目录、图片处理、代码 Agent、本地文件系统 |
| `routers.batch_workflow` | `/api/plan`、`/api/plan/review`、`/api/ws/batch` | 批量规划与批量资产创建 | 面向本地 Mod 工程生成流程 |
| `routers.config_router` | `/api/config/*` | 工作站配置、路径探测、本机 AI 能力、平台队列诊断、生图测试 | 依赖本机配置和本地环境 |
| `routers.knowledge_router` | `/api/knowledge/*` | 运行时知识库状态检查与刷新 | 服务本地 STS2 知识库与代码生成上下文 |
| `routers.log_analyzer` | `/api/ws/analyze-log` | 读取本机游戏日志并交给 LLM 分析 | 本地诊断能力 |
| `routers.mod_analyzer` | `/api/ws/analyze-mod` | 扫描本地 Mod 项目源码与 localization 内容并分析 | 本地项目分析能力 |
| `routers.build_deploy` | `/api/ws/build-deploy` | 构建项目并部署到本机 STS2 Mods 目录 | 直接依赖本地构建链路和游戏安装目录 |
| `routers.approval_router` | `/api/approvals/*` | 审批请求读取、批准、拒绝、执行 | 当前归属工作站路由，服务本地工作流执行链路 |

### 4.2 Web 路由集合

| 路由模块 | 典型路径 | 作用 | 边界判断 |
| --- | --- | --- | --- |
| `routers.auth_router` | `/api/auth/*` | 注册、登录、登出、当前登录态、邮箱验证、找回密码 | 平台身份体系 |
| `routers.me_router` | `/api/me/*` | 当前用户 profile、quota、jobs、server preferences、upload assets、server workspaces | 当前用户视角的平台 API |
| `routers.platform_jobs` | `/api/platform/jobs/*`、`/api/platform/quota`、`/api/platform/execution-profiles` | 平台任务、任务事件、配额、执行配置兼容接口 | 数据库驱动的平台任务接口 |
| `routers.platform_admin` | `/api/admin/*` | 执行记录、退款、审计、服务器凭据、执行配置管理 | 平台后台管理能力 |

当前 `app_factory.create_app(role)` 的装配规则是：

- `role == "workstation"`：只加载 `WORKSTATION_ROUTER_MODULES`，并挂载前端静态资源。
- `role == "web"`：只加载 `WEB_ROUTER_MODULES`，不挂载前端静态资源，并注册 Web 队列 worker 生命周期。

## 5. 工作站主要模块关系

```mermaid
flowchart TB
    subgraph R["工作站路由层"]
        WF["routers.workflow\n/api/ws/create\n/api/project/*"]
        Batch["routers.batch_workflow\n/api/plan\n/api/ws/batch"]
        Cfg["routers.config_router\n/api/config/*"]
        K["routers.knowledge_router\n/api/knowledge/*"]
        Log["routers.log_analyzer\n/api/ws/analyze-log"]
        Mod["routers.mod_analyzer\n/api/ws/analyze-mod"]
        Build["routers.build_deploy\n/api/ws/build-deploy"]
        Approval["routers.approval_router\n/api/approvals/*"]
    end

    subgraph Core["本地业务能力"]
        WorkflowEngine["app.modules.workflow\nWorkflowEngine / SingleAsset / BatchAsset"]
        Planning["app.modules.planning\nplan_mod / validate / execution_bundles"]
        Codegen["app.modules.codegen\nPromptAssembler / CodegenService"]
        Image["image + app.modules.image\ngenerate / postprocess / prompt_adapter"]
        Knowledge["app.modules.knowledge\nSTS2 知识 / BaseLib / guidance"]
        ApprovalCore["approval/*\nstore / service / executor"]
        ProjectUtils["project_utils\n创建项目 / 路径检测 / local props"]
    end

    subgraph LLM["AI 执行层"]
        Text["llm.text_runner\nLiteLLM / Claude CLI / Codex CLI"]
        Agent["llm.agent_runner\nClaude CLI / Codex CLI"]
    end

    WF --> WorkflowEngine
    WF --> Image
    WF --> Codegen
    WF --> ApprovalCore
    Batch --> Planning
    Batch --> WorkflowEngine
    Batch --> Image
    Batch --> Codegen
    Cfg --> ProjectUtils
    K --> Knowledge
    Log --> Text
    Mod --> Text
    Build --> Codegen
    Approval --> ApprovalCore
    Codegen --> Knowledge
    Codegen --> Agent
    Planning --> Text
    Image --> Text
```

工作站侧的判断标准：

- 要操作本地项目目录、本地配置、本机日志、STS2/Godot/Mods 路径。
- 要通过 WebSocket 给工作台页面推送长流程状态。
- 要调用本地 CLI 代码代理或构建部署链路。
- 要服务单人创作工作流，而不是平台多用户任务管理。

## 6. Web 主要模块关系

```mermaid
flowchart TB
    subgraph API["Web 路由层"]
        Auth["auth_router\n/api/auth/*"]
        Me["me_router\n/api/me/*"]
        Jobs["platform_jobs\n/api/platform/*"]
        Admin["platform_admin\n/api/admin/*"]
    end

    subgraph App["平台应用服务"]
        AuthSvc["AuthService"]
        UserCenter["UserCenterService"]
        JobApp["JobApplicationService"]
        JobQuery["JobQueryService"]
        Orchestrator["ExecutionOrchestratorService"]
        Quota["QuotaBillingService"]
        Routing["ExecutionRoutingService"]
        ServerExec["ServerExecutionService"]
        Workspace["ServerWorkspaceService + Lock"]
        Upload["UploadedAssetService"]
        Audit["PlatformRuntimeAuditService"]
        Queue["ServerQueuedJobWorker / Claim / ScanClaim"]
        Cred["ServerCredentialAdmin / Cipher / HealthChecker"]
    end

    subgraph Runner["平台执行 Runner"]
        Registry["PlatformWorkflowRegistry"]
        RunnerSvc["WorkflowRunner"]
        Dispatcher["StepDispatcher"]
        Adapter["ExecutionAdapter"]
        Handlers["step handlers\ntext / log / code / asset / build"]
    end

    subgraph Persistence["持久化层"]
        DB["SQLAlchemy session"]
        Repos["Job / Event / AIExecution / Artifact\nQuota / Usage / Credential / Routing repositories"]
    end

    Auth --> AuthSvc
    Me --> UserCenter
    Me --> JobApp
    Me --> ServerExec
    Me --> Workspace
    Me --> Upload
    Jobs --> JobApp
    Jobs --> JobQuery
    Jobs --> ServerExec
    Admin --> Audit
    Admin --> Cred
    Admin --> ServerExec

    JobApp --> Orchestrator
    JobApp --> Queue
    Orchestrator --> Quota
    Orchestrator --> Routing
    Orchestrator --> Workspace
    Orchestrator --> Upload
    Orchestrator --> Registry
    Orchestrator --> RunnerSvc

    Registry --> RunnerSvc
    RunnerSvc --> Dispatcher
    Dispatcher --> Adapter
    Adapter --> Handlers

    AuthSvc --> DB
    UserCenter --> DB
    JobApp --> DB
    JobQuery --> DB
    Orchestrator --> DB
    Admin --> DB
    DB --> Repos
```

Web 侧的判断标准：

- 要管理 `Job`、`AIExecution`、`Quota`、`Audit`、`Credential` 等平台数据。
- 要使用数据库会话、仓储、查询服务或平台应用服务。
- 面向多用户、多任务、服务器运行态。
- 不依赖前端静态文件托管，也不应直接读取用户本机路径。

## 7. 主链路关系

```mermaid
sequenceDiagram
    participant FE as 前端
    participant W as workstation 路由
    participant Img as 图像/后处理
    participant Code as Codegen
    participant Agent as Claude/Codex CLI
    participant Web as web 路由
    participant Job as JobApplicationService
    participant Orch as ExecutionOrchestrator
    participant Run as Platform Runner
    participant DB as 数据库

    FE->>W: 本地生成 / 构建 / 分析 WebSocket
    W->>Img: 生图、处理图片
    W->>Code: 组装 STS2 知识与代码 Prompt
    Code->>Agent: 写代码、构建、修复
    Agent-->>W: 流式输出
    W-->>FE: stage / stream / done

    FE->>Web: 登录、用户中心、服务器模式任务
    Web->>Job: create / start / cancel job
    Job->>DB: 写 job / item / event
    Job->>Orch: 启动 AI execution
    Orch->>DB: 扣配额、写 execution
    Orch->>Run: 执行已注册 workflow steps
    Run->>Agent: text / code / asset / build handler
    Run-->>Orch: StepExecutionResult
    Orch->>DB: 写结果、artifact、event
    Web-->>FE: job detail / events / quota
```

## 8. 共享模块与特殊模块

### 8.1 共享模块

以下模块可被两端复用，但不能因此模糊归属：

- `llm/*`：文本补全、Claude/Codex CLI 代码代理。
- `app.modules.codegen`：代码生成 prompt 组装、代码写入、构建触发。
- `app.modules.knowledge`：STS2 / BaseLib 知识上下文。
- `app.shared.*`：配置、契约、错误、prompt、基础设施。

共享模块的归属判断看“入口和运行语义”，不是看目录是否被两边 import。

### 8.2 审批模块

`approval_router` 当前仍在工作站路由集合中，使用 `approval.runtime`、`approval.store`、`approval.service` 和 `approval.executor`。平台侧存在 `ApprovalAdapter` / facade 方向的演进痕迹，但当前 `web` 路由集合没有挂载 `/api/approvals/*`。

因此审批能力当前准确表述为：

- 路由归属：工作站端。
- 主要服务对象：本地工作流执行链路。
- 后续若要平台化，需要迁入 Web 端并补齐数据库、权限、审计和跨用户隔离边界。

## 9. 新增接口归属规则

应归到工作站端：

- 要操作用户本机项目目录、日志、配置、STS2/Godot/Mods 路径。
- 要驱动本地构建、部署、图片处理或本地 CLI 工作流。
- 要给工作台页面做 WebSocket 长流程推流。

应归到 Web 端：

- 要管理用户、任务、执行、配额、凭据、审计。
- 要通过数据库持久化查询或命令。
- 要面向服务器部署、平台队列、平台后台或多用户运行态。

需要单独评审的混合接口：

- “工作站触发，但平台落库”的流程。
- 审批相关接口。
- Build/Deploy 的平台化改造接口。
- 既想本地运行又想服务器运行的兼容接口。

## 10. 当前建议

后续默认采用以下口径：

- “本机工作站端后端” = `create_app("workstation")` + `WORKSTATION_ROUTER_MODULES` + 本地工作流。
- “Web 端后端” = `create_app("web")` + `WEB_ROUTER_MODULES` + 数据库 + 平台应用服务 + 队列 worker。
- “Web 工作站执行面” = Docker 内网中的 `create_app("workstation")`，只接受 `web` 通过 token 调度。
- `backend/main.py` 不是 `full`，而是工作站入口兼容文件。
- `full`、迁移开关条件挂载等说法只应作为历史阶段记录，不再作为当前实现基线。
- 部署与打包入口只使用 `tools.ps1 package app`、`tools.ps1 deploy app`、`tools.ps1 stop app`。
