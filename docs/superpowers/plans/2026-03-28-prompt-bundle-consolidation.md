...
Run: `.\backend\.venv\Scripts\python.exe -m pytest .\backend\tests\test_code_agent_api_lookup.py .\backend\tests\test_codegen_module.py -k "api_lookup" -v`
...
要求：
- 从 `codegen.md` 读取 `api_lookup_title`、`api_lookup_baselib`、`api_lookup_sts2_local`、`api_lookup_sts2_fallback`
