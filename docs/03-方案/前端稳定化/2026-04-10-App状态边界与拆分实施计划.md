...
- [ ] **Step 1: 写定向 failing tests**
  验证单资产入口在拆分后仍能完成输入、启动 workflow、选择图片、审批继续和 reset。

- [ ] **Step 2: 创建 `SingleAssetWorkspaceContainer`**
...
**Commands:**
- `npm test -- --runInBand frontend/tests/singleAssetController.test.ts`
- `npm test -- --runInBand frontend/tests/singleAssetState.test.ts`
...
- [ ] **Step 1: 写定向 failing tests**
  验证本机执行 / 服务器执行、登录跳转、知识库缺失确认和平台任务创建仍保持原行为。

- [ ] **Step 2: 抽出 `useExecutionModeFlow`**
  统一封装：
  - `pendingExecution`
...
- [ ] **Step 3: `App.tsx` 改为消费 hook 返回值**
  顶层只负责把结果接到全局弹窗入口。

- [ ] **Step 4: 运行定向测试**

**Commands:**
- `npm test -- --runInBand frontend/tests/executionModeDialog.test.ts`
- `npm test -- --runInBand frontend/tests/platformRunFlow.test.ts`
...
- [ ] **Step 2: 抽出 `useKnowledgeStatusFlow`**
  封装：
  - `knowledgeStatus`
...
- [ ] **Step 3: 在 `App.tsx` 中移除知识库轮询细节**
  顶层仅消费 hook 提供的最终状态和事件。

- [ ] **Step 4: 运行定向测试**

**Commands:**
- `npm test -- --runInBand frontend/tests/knowledgeWorkflowBanner.test.ts`
...
- [ ] **Step 1: 写/改定向 failing tests**
  验证：
  - 路由可正常进入首页、设置页、认证页、用户中心页
  - 工作区壳层仍能正确切换 tab

- [ ] **Step 2: 把 `renderWorkspaceContent()` 收口为独立组件**
  避免 `App.tsx` 同时充当路由器和内容编排器。

- [ ] **Step 3: 清理无关局部状态与多余 helper**
  确保顶层只保留真正全局的状态和 handler。

- [ ] **Step 4: 运行定向测试**

**Commands:**
- `npm test -- --runInBand frontend/tests/feature-shell.test.ts`
- `npm test -- --runInBand frontend/tests/settingsPageRoute.test.ts`
