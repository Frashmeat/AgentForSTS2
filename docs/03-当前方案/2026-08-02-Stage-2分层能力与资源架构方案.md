...
| 实施进度 | Work Order 1-4 已完成 Kernel/DAG、Run/Artifact v3、Pack/Contribution、Truth/Evidence、Resource Workspace、ModelRequestSnapshot 和 `log.analyze` 纵切面；旧生产链尚未切换 |
...
- 对 Core 已注册 provider、runner、indexer、validator 和 transformer 的 ID 引用。
...
当前 Shell 仍使用旧 handler，统一切换和旧 Prompt 删除保留到 Work Order 7。

### Work Order 5：单 Mod 与资源纵切面

- 迁移 `mod.plan`、`mod.generate.single` 和 `resource.prepare`。
- 拆分 `codegen.md` 的 Feature、Pack 和 Truth 内容。
- 接入统一资源版本、选择、验证和 provenance。

验收：完成一条 STS2 单项生成、compile、Artifact 发布主链；Prompt、Evidence 和资源来源可复验。

### Work Order 6：组合 Feature

- Batch 改为组合 Single。
- Complex 改为组合 Plan、Batch、Resource、Build 和 Package。
- Build/Package 使用 Pack recipe 和 Runtime runner。

验收：不存在平行 codegen、文件事务或构建实现；失败和取消沿组合图正确传播。
