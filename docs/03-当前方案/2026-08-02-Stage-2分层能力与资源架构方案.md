...
> 文档定位：本文记录已完成的 Game Pack Stage 2 基础架构方案；2026-08-04 起的 Item/Resource 扩展以 [`当前方案`](./当前方案.md) 和当前 Trellis 父任务为准。
>
...
| 项 | 决策 |
| --- | --- |
| 状态 | Stage 2 Work Order 1-8 已完成并通过最终安装态验收 |
| 目标形态 | 编译期分层的模块化单体 |
...
| 实施进度 | 最终候选 `rc-20260803T125802Z-bde2d64969d9` 已验收；后续 Pack v3 Item/Resource 扩展属于新任务 |
...
Composition Plan 只产生结构化 Item 内容和 references，不让模型选择 Resource。Plan 后的 Draft
node 在 Shell 中复用通用 Resource Workbench：角色来自 Pack capability，candidate 的
prepare/select 仍由 Resource Workspace 持久化，所选绑定则通过 Draft revision-CAS 写回该
node 的 ItemDefinition。Ready confirmation 在资源选择完成后再次权威校验，未就绪时返回
`composition.confirm.not_ready` / `replace_resource`，并且不移动任何 Item current pointer。
...
HTTP Adapter 必须在 Run 前显式选择 Provider-native 格式。Anthropic 使用
`output_config.format.json_schema`；OpenAI-compatible 默认使用
`llm.openai_response_format=json_schema`，只有经实测忽略/拒绝 JSON Schema 但支持
JSON Object 的代理才显式改为 `json_object`。后者仍在 Recipe Prompt 中携带完整
typed contract，并由 Feature 执行同一严格反序列化/验证。一个 Run 不得在失败后自动
降级格式重发，也不得剥离 Markdown、提取第一个对象或伪造成功。

Plan 不负责生成 Truth 索引键。`mod.plan` result v2 输出描述性的
`evidenceRequirements`；当前 Pack v3 顶层 item catalog 为每个类型提供结构化
`evidenceQueries { symbols, terms }`。Capability readiness 与 Single Generate 对每组 Pack 查询
强制至少一个 verified Truth 命中，并在总上限内去重装配。不得把 Plan 自然语言拆词猜测为
symbol，也不得要求用户输入内部索引键才能生成。
...
2026-08-04 的后续 Pack 扩展将该组合合同收紧为 Batch request v4/result v2 与 Complex request v3/result v2。Batch 输入固定 `StoredItemDefinition` snapshot/hash，由 Feature 为每项执行并持久化 Plan→Single；父 Run 成功只表示编排完成，Item typed failure 留在 result。Complex 直接复用同一 Batch，只在全部 Item 成功后执行 Build/Package。Shell 只提供 Item Library 选择、只读 typed preview、persisted child Run 和基于父 request snapshot 的精确失败重试，不再提供可编辑 JSON 或第二套 planning loop。
...
首轮安装版复验发现旧 `requiredEvidence: string[]` 同时承担业务描述和精确 symbol 查询，普通
模型输出因此在 Generate 前失败。WO8 修复将其破坏性拆分为 Plan v2
`evidenceRequirements` 和 Pack contribution v2 `evidenceQueries`，并要求 Single/Batch 保留
Truth、Pack、Resource、Model、Validation、Artifact、Run 的 typed failure 分类。修复后的 Pack
hash、Truth Snapshot、新候选和安装证据必须重新绑定，首轮候选只保留为缺陷发现证据。
