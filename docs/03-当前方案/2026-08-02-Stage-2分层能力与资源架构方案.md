...
Direct Single 仍是单次严格调用。Composition Generate request v5 才启用 Graph-owned semantic feedback 和单 Item adjustment；Shell 只接收有界 View，不接收 feedback envelope、checkpoint 或 raw completion，普通 UI 也不展示内部 round/phase。
...
Composition Generate request v5 把可修订 output-contract diagnostic、registered validator 的多 Item generated-content campaign 和版本绑定的单 Item adjustment 纳入同一 Feature-owned controller。模型返回完整 bundle/role set，每轮重新严格 decode，并在 checkpoint 后重建 finalize、重跑完整 validator suite。Graph 总预算不按节点放大，`until_passed` 也有 20 次绝对上限。成功结果调整派生新 Graph/Run/Artifact version并保留源证据；paused pre-commit Graph才允许原地 CAS。`commit_prepared` 只能在 validation 成功后创建。
