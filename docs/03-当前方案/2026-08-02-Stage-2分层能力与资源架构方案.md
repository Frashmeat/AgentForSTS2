...
Direct Single 仍是单次严格调用。Composition Generate request v4 才启用 Graph-owned semantic feedback；Shell 只接收聚合 round 和闭集 phase，不接收 feedback envelope、checkpoint 或 raw completion。
...
### ExecutionGraphRecord v3

大型 Plan/Generate 的可恢复状态。Graph 保存 Blueprint identity、nodes、attempt count/active attempt、当前 normalized checkpoint、versioned/hashed safe feedback state、active/previous Runs、graph failure、commit intent 和 final result ref。feedback state 记录 Graph-total round、phase、typed diagnostic fingerprint、candidate SHA 和/或 checkpoint hash，不保存 raw completion。一个 graph 最多一个 active Run。v3 破坏性替换 v2，只读取 `.ats/execution-graphs-v3`，不迁移旧 graph、不 dual read、不保留历代生成文件。

Composition Generate request v4 把可修订 output-contract diagnostic 与 registered validator 的 generated-content issue 纳入同一 Feature-owned controller。Feature 只修订可信合同错误或可唯一归因到一个非 merge 生成文件的 Item；模型返回完整 bundle/role set，每轮重新严格 decode，并在 checkpoint 后重建 finalize checkpoint、重跑完整 validator suite。Graph 总预算不按节点放大，`until_passed` 也有 20 轮绝对上限。`commit_prepared` 只能在 validation 成功后创建。
