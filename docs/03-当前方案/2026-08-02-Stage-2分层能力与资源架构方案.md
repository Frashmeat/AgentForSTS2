...
### ExecutionGraphRecord v2

大型 Plan/Generate 的可恢复状态。Graph 保存 Blueprint identity、nodes、attempt count/active attempt、当前 normalized checkpoint、repair round、最近 diagnostic fingerprint/checkpoint hash、active/previous Runs、graph failure、commit intent 和 final result ref。一个 graph 最多一个 active Run。v2 破坏性替换 v1，不迁移旧 graph、不 dual read、不保留历代生成文件。

Registered validator 返回最多 256 个 typed issues，并区分 generated content、local environment 与 non-repairable。Feature 只修订可唯一归因到一个非 merge 生成文件的 Item，模型返回该 Item 全部声明 role 的完整替换内容；每轮替换后重建 finalize checkpoint 并重跑完整 validator suite。`commit_prepared` 只能在 validation 成功后创建。
