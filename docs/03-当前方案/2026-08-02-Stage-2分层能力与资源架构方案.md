...
| 项 | 决策 |
| --- | --- |
| 状态 | Work Order 8 repair after installed-app Evidence contract finding |
| 目标形态 | 编译期分层的模块化单体 |
...
| 实施进度 | Work Order 1-7 已完成；WO8 首轮门禁/候选完成，安装版 E2E 触发 Plan v2 + Pack Evidence Query v2 修复，等待重新门禁和候选 |
...
`evidenceRequirements`；`pack.mod-generate-single` v2 在每个 item type 中提供结构化
`evidenceQueries { symbols, terms }`。Single Generate 对每组 Pack 查询强制至少一个 verified
Truth 命中，并在总上限内去重装配。不得把 Plan 自然语言拆词猜测为 symbol，也不得要求用户
输入内部索引键才能生成。
