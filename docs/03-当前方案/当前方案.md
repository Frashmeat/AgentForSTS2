...
可恢复生成与 validation-guided repair 主体已提交。绑定 `b68c33df` 的 candidate 已通过 release 10/10、安装和运行时身份核对，但随后 GUI E2E 暴露旧 ExecutionGraph 路径和 project-local 工具输入回退缺陷；fallback 已提交为 `659a04c3` 并通过 225/225 workspace tests、完整机器门禁和隔离 GUI E2E 8/8，因此旧 candidate 已失效。O8 仍需构建、安装绑定当前最终 HEAD 的新 candidate，执行 fresh Provider closure，并由用户完成真实 STS2 单人验收。
...
`b68c33df` 已提交 validation-guided repair；candidate `rc-20260812T084008Z-b68c33df4e3b` 通过 release 10/10、安装态 BuildInfo/12-Feature 核对并配置为 `deepseek-v4-flash/json_object`。隔离 GUI 随后暴露 v1 E2E 路径残留和“全局 Godot 清空覆盖 project-local Build”两个缺陷；当前 fallback 最终树已通过完整机器门禁和保留证据的 GUI E2E 8/8，因此该 candidate 已失效。剩余顺序：

1. 构建并安装绑定当前 fallback 提交的 replacement ML candidate，核对 BuildInfo、variant、features、catalog 和 Provider format。
2. 创建全新物理 Truth/Project/Draft/Run/Item/Artifact/Package 证据。
