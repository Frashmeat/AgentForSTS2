...
本轮允许破坏性合同升级，不保留会形成双重事实来源的临时兼容层。每个 Order 完成受影响机器门禁后单独提交；最终集中执行新候选与人工安装验收。
...
- Order 1：Pack/Item contracts、ItemDefinition/hash、Truth capability readiness。
- Order 2：Item repository、Item Library、通用混合编辑器、英中 confirmed/outdated。
- Order 3：真实图片探测、master/derived、deterministic transform、candidate/selected。
...
- 每个 Order 先跑受影响测试、workspace check/test/clippy、DAG、fmt/diff；Git commit 单独授权。
