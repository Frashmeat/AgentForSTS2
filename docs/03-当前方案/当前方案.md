...
candidate `rc-20260815T133556Z-dbe47d958794` 已绑定提交 `dbe47d95`，完成 release 10/10、baseline/ML 双变体验证、ML 安装、BuildInfo 核对和 fresh closure 前置准备。其 Generate Graph 在首个 Single 节点因非流式长请求传输失败而暂停；当前工作树已完成通用 SSE 修复和机器门禁，因此该 candidate 及 closure 只保留诊断价值，下一轮必须重新构建 candidate 和物理验收根。
...
1. [待授权] 提交 SSE 修复后，构建并安装绑定最终提交的新 ML candidate；`dbe47d95` candidate 只保留诊断价值。
