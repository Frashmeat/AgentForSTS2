...
O8 的真实 Provider 验证证明单次整图 Character Plan 会形成约 20 分钟长连接；即使使用共享 FIFO、显式 `json_object` 和 300/600 秒退避，整个 11-node 响应仍可能因传输失败全部作废。用户于 2026-08-10 确认采用跨游戏的“Runtime 通用执行图 + Feature 分阶段编排 + Pack 声明游戏差异 + Adapter 可靠传输/持久化”方案；该方案已经进入当前工作树。详细合同、恢复语义和实现状态见 [`可恢复分阶段 Mod 生成架构方案`](./2026-08-10-可恢复分阶段Mod生成架构方案.md)。代码变化已使历史 candidate 失效，完整机器门禁和新 candidate 仍待执行。
...
- Order 8（进行中）：历史 candidate `rc-20260809T152710Z-9d0e390a513a` 保留 10/10 release 与安装态长输出失败证据，但已被当前 staged-generation 代码和 Pack SHA 变更作废。当前工作树已实现 ExecutionGraph v1、Draft v2/createOrMatch、Pack guidance v2、Blueprint/Suite Brief/串行节点 checkpoint、本地 reference binding、ProjectSession 恢复、Tauri execution controls 和 React progress。GUI E2E 发现并修复了零计数 target group 写入空 reference slot 的合同冲突。当前 workspace 199 tests、workspace check/clippy、desktop feature checks、TypeScript/frontend/build、DAG、CLI 12 Features、release/build-plan script tests、fmt/diff 和隔离 GUI E2E 7/7 全部通过；新 candidate、全新安装态 acceptance evidence 和真实 STS2 单人验收仍待执行。

O8 的稳定性修复阶段已经完成代码、本地机器门禁和隔离 GUI E2E：项目内 execution graph 将整图请求拆成 Pack-driven Blueprint、Suite Brief、串行节点请求、本地 reference binding、整图验证和原子 Draft create，并保留共享 FIFO 与小型 Single Request 快速路径。保留证据还证明 Pause/Resume/Cancel 状态、invalid-node 显式恢复只增加该节点一次请求、Draft v2 provenance 与零 transaction/原子写残留。下一步是在明确授权后构建绑定当前提交的新 candidate，然后使用全新 Truth/Project/Draft/Run/Item/Artifact/Package 进入安装态 Provider 与真实游戏验收。
