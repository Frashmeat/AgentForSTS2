...
可恢复生成主体和上一轮机器门禁已经完成。安装态 Provider closure 随后暴露了 definition-bound Plan 的事实漂移：模型 Plan 与已确认 Character definition 在 HP、Starter Relic 和 Card 集合上冲突。当前修复使旧 candidate 失效；O8 需要先完成新代码门禁，再构建新 candidate，最后执行安装态 closure 和用户负责的真实 STS2 单人验收。
...
`rc-20260811T023832Z-51e0ae6b61dc` 已安装并形成真实失败证据，但它绑定旧代码，不能继续代表当前修复。剩余顺序：

1. 修复提交 `235696d6` 的完整机器门禁和隔离 E2E 已通过。
2. 取得明确 candidate 构建授权，构建并验证绑定 `235696d6` 的 candidate。
3. 取得明确安装授权，安装 ML candidate 并核对运行时 BuildInfo、variant、features 和 catalog。
