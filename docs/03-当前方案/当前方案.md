...
本轮允许破坏性合同升级，不保留会形成双重事实来源的临时兼容层。Order 1-3 已完成 Pack/Item/readiness、版本化 Item Library、通用编辑器和 Resource v2 后端地基；当前从 Order 4 开始建立 Resource Workbench 并完成 Relic 纵向 Gate。每个 Order 完成受影响机器门禁后自动提交；最终集中执行新候选与人工安装验收。
...
- Order 4（下一步）：可视化 Resource Workbench，并用 Relic 通过真实纵向 Gate。
