...
`ats-features` 定义产品能做什么：Plan、Single/Batch/Complex/Composition Mod generation、Resource prepare、Build、Package、Log analyze。功能扩展发生在这里并要求所有 Pack 补齐相应 Contribution；游戏类型扩展不新增通用 Feature，也不复制工作流。Composition generation 只提升 publication transaction 的范围，继续复用现有 Plan、Single proposal、Build 与 Package 服务。
