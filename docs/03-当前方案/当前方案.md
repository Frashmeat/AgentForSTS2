...
- Order 8（机器门禁已完成）：修复安装态 Provider 验证发现的 Plan run-scoped 合同、失败详情、Studio 失败 Run 保留、Branded guidance 和桌面配置五项问题，并通过修复后完整机器门禁；新 Windows candidate、安装态 E2E 和全新存档的 STS2 单人可玩闭环仍待执行。
...
- `e00f0fa8` 候选只证明上一轮 Pack v3 / ItemDefinition v1；`rc-20260806T161945Z-366203d42b3e` 只证明修复前 Pack/代码并保留为缺陷证据。O8 最终验收必须使用绑定当前提交的新 candidate、新 Pack v4 Truth、新 Item graph、Run/Artifact/Package 和新游戏存档，不得复用或重命名旧证据。
