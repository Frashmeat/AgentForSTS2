...
所有旧 candidate 和失败证据只保留诊断价值；当前进入 O9 授权边界，尚未构建或安装绑定 Provider foundation 最终代码的新 candidate。
...
1. 取得明确授权后，构建并安装绑定最终修复和文档同步 HEAD 的新 ML candidate。
...
完整 checklist、最新 Run/Graph 身份和失败证据见本地 Trellis 任务的 `acceptance.md`。candidate 构建、安装和应用启动尚需明确授权；真实游戏 UI 仍只由用户操作。
