...
candidate `rc-20260815T133556Z-dbe47d958794` 已绑定提交 `dbe47d95`，完成 release 10/10、baseline/ML 双变体验证、ML 安装、BuildInfo 核对和 fresh closure 前置准备。其 Generate Graph 在首个 Single 节点因非流式长请求传输失败而暂停；通用 SSE 修复已提交为 `ec8b3311` 并通过机器门禁，因此旧 candidate 及 closure 只保留诊断价值，下一轮必须重新构建 candidate 和物理验收根。
...
1. [待授权] 构建并安装绑定 `ec8b3311` 的新 ML candidate；`dbe47d95` candidate 只保留诊断价值。
2. 为新 candidate 创建全新物理 Truth、Project、Draft、Items、Resources、Run、Graph、Artifact 和 Package 证据。
...
完整 checklist、最新 Run/Graph 身份和失败证据见本地 Trellis 任务的 `acceptance.md`。新 candidate 构建、安装和 installed acceptance 启动仍保持独立授权，真实游戏 UI 只由用户操作。
