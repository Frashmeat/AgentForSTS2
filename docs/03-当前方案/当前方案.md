...
本轮允许破坏性合同升级，不保留会形成双重事实来源的临时兼容层。Order 1-8 已完成通用地基、Relic/Card/Potion/Power、Batch/Complex 和最终机器门禁；绑定 `e00f0fa8` 的新 Windows 候选也已完成安装态 E2E 并由用户明确确认验收。下一阶段不是继续补本轮地基，而是讨论下一批类型与资源能力；真实外部 Provider、AI 图片质量、真实 STS2 游戏行为和签名/发布是独立后续门禁。
...
资源采用 ResourceAsset v2 + master + deterministic derivation。Upload、AI 和 Pack default 都先产生 `selectedVersion=null` 的不可变 candidate；真实 PNG 探测、派生和预览成功后由用户显式确认 selected。Pack v2 Resource graph 只引用注册的 v1 transform Primitive；派生记录 source role/version、transform primitive/version/parameters hash、Pack identity 和输出 hash。一个 master 的全部派生使用同一 staging batch，任一 candidate 失败不得改变既有 selected。ProjectSession 持有唯一 Resource repository；Single 只接受 exact media shape 且已显式 selected 的精确版本。
...
Pack v3 顶层 item catalog 已消除 Plan、Single 和 UI 三处类型/Resource/Truth 事实重复。`pack.mod-plan-guidance` v3 只保留规划指导；`pack.mod-generate-single` v4 分离 common guidance 与当前 item guidance，并保留 validation Primitive 和 exact generated-file roles。
...
- Order 3（已完成）：真实 PNG 探测、master/derived、deterministic transform、candidate/selected 后端合同。
- Order 4（已完成）：可视化 Resource Workbench，并用 Relic 通过真实纵向 Gate。
- Order 5（已完成）：在同一地基上增加 Card 的 Pack/Truth/Recipe 内容和纵向测试。
- Order 6（已完成）：在同一地基上增加 Potion 的字段、Truth、icon、本地化和纵向 Gate。
- Order 7（已完成）：增加 Power 的类型/叠层/实例字段、生命周期 Truth、双图资源和纵向 Gate。
- Order 8（已完成）：Item Library Batch v4/Complex v3、Plan/Single child Run、精确失败重试、真实 Build/Package、GUI E2E 和全量机器门禁。
...
- 绑定 `e00f0fa8` 的新完整 release candidate 和安装态 E2E 已验收；后续代码变更后仍必须使用新候选、新 Truth、新 Item/Run/Artifact，不得复用本轮证据。
- 真实外部 Provider、AI 图片质量、真实 STS2 游戏行为、签名/SmartScreen 和发布未纳入本轮验收，仍需独立授权。
