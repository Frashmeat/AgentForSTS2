...
可恢复生成、validation-guided repair 和统一 Generation Feedback Controller 已提交为 `3a15bd03`。绑定该提交的 candidate `rc-20260812T162726Z-3a15bd03ded9` 已通过 release 10/10、安装身份和 12-Feature catalog 核对，并已实际执行 fresh installed Provider closure。

该 closure 的 23/23 生成 nodes 完成，Character 首次 output-contract 错误由统一反馈闭环自动恢复；随后 whole-closure validation 发现 11/11 生成文件共有 111 个真实 C# 编译错误。Run 终态为 `failed / validation.rejected`，Graph 为 `paused / validation.not_repairable`，没有发布 Artifact、ZIP 或部分工程。当前根因是单 owner repair 合同拒绝 cross-owner diagnostics，不是尚未执行 Provider closure。

当前执行顺序已调整为：O8 实现多 Item 自动技术修复与版本绑定的单 Item 用户调整，并通过机器门禁；O9 再构建新 candidate，完成 fresh installed closure 和用户真实 STS2 单人验收。O8 方案已认可，尚未实现。
...
安装态 O8 closure 进一步证明单 Item output-contract feedback 可以自动恢复，同时暴露了批次生成的下一层缺口：多个 Item 同时产生 generated-content 编译错误时，当前单 owner repair 会安全暂停，且普通用户不应承担技术诊断和修复调度。已认可的后续方向是由系统内部按 Item ownership 和依赖顺序串行修复技术错误，只向用户展示生成、检查、自动调整和预览状态；生成后的创作调整必须绑定一个具体 Item 和当前 definition 版本，调整后重新验证完整组合。该方向尚未实现，详见 [`批次生成自动修复与单项调整架构方案`](./2026-08-13-批次生成自动修复与单项调整架构方案.md)。
...
1. 定义多 Item ownership、依赖有序 repair campaign、持久化 cursor 和 bounded typed evidence。
2. 将单 owner repair 重构为共享 FIFO 下的串行多 target 修复；每轮完成后重新验证完整闭包。
3. 增加 `itemId + expectedDefinitionHash` 绑定的单 Item 用户调整；只重生成目标 Item，随后完整复验。
4. 普通 UI 只展示生成、检查、自动调整、需要处理和预览，不暴露 Graph、Run、compiler code 或 repair budget。
5. 同步 stable specs，完成 focused、workspace、frontend、DAG、catalog 和 isolated GUI E2E 门禁。

### O9 - 新候选与真实验收

1. 在明确授权后构建并安装绑定 O8 最终提交的新 ML candidate。
2. 创建全新物理 Truth、Project、Draft、Items、Resources、Run、Graph、Artifact 和 Package 证据。
...
完整 checklist、最新 Run/Graph 身份和失败证据见本地 Trellis 任务的 `acceptance.md`。candidate 构建、安装和真实游戏 UI 不属于当前文档整理授权。
...
- [`批次生成自动修复与单项调整`](./2026-08-13-批次生成自动修复与单项调整架构方案.md)：多 Item 自动修复、技术复杂度隐藏和生成后单 Item 调整。
