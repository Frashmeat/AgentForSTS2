...
- Order 8（进行中）：`rc-20260808T153706Z-a0ea644832f7` 已通过 10/10 release steps、安装身份和隔离 Truth/工程前置验证，但真实 `qwen3.7-plus` Plan 在三次有界 HTTP 尝试后以 `model.transport_failed` 终止；该长 Run 暴露前端固定 180 秒轮询早于后端合法预算结束。共享轮询已改为默认等待持久化终态并加入显式 deadline 回归测试，旧 candidate 因代码变化失效；新 candidate、成功 Draft/Item/Artifact/Package 和全新存档的 STS2 单人可玩闭环仍待完成。
