...
当前未提交的模型输出预算工作树已通过 workspace 258/258、frontend 41/41、workspace
check/clippy、desktop `ml-rembg`/`e2e` feature checks、TypeScript、production build、DAG、12-Feature
CLI catalog 和 diff-check。此前 isolated GUI E2E 8/8 属于现有 candidate 的已保留证据，本次未构建
replacement candidate，也未重跑 installed GUI E2E。

当前 candidate `rc-20260816T084051Z-b9d4547665be` 精确绑定提交 `b9d4547665be6d916a1ede279875356eb5d795ac`，完成 release 10/10、baseline/ML 双变体验证、四个 installer 独立 hash 复算、ML 安装和 BuildInfo 核对。它包含 `ec8b3311` 的通用 SSE 修复。fresh installed closure 已准备全部 11 Items 与 Resources，但唯一 Graph 在第一个 Single 节点因代理拒绝 `16,384` 输出预算而暂停；该旧 candidate 的 Adapter 还把 HTTP 403 误分类为鉴权失败。当前工作树已修复两项缺口，但尚未提交或形成 replacement candidate。
...
Single 多文件 bundle 的 Recipe 业务上限为 16,384 tokens；它不等于所有模型/代理的实际请求
能力。System 高级设置提供可选 `llm.max_output_tokens`，Feature 在创建
`ModelRequestSnapshot` 前按
`min(recipe.maxOutputTokens, configured llm.max_output_tokens)` 解析有效预算。snapshot/hash、
Runtime request 与 HTTP body 必须使用同一有效值，Adapter 不得发送前静默截断。未配置时保持
Recipe 值。`model.output_invalid` 和 `model.output_truncated` 只附带闭集 `reasonCode`，不持久化
raw completion、解析器文本、Provider body 或模型生成的未知 role。

Direct Run 在执行开始时解析当前 Settings；staged Plan/Generate 分别使用 Blueprint v2/v7，在
Graph 创建时固定 validated limits。Resume 只使用 blueprint 中的 pinned limits，当前 Settings
变化不会重写未完成节点的 snapshot/hash；要采用新上限必须创建新 Graph。

HTTP 401 映射 `model.authentication`；HTTP 403 默认映射 `model.request_rejected`。除非未来获得
可信、明确的 invalid-key 结构化证据，不能把 403 猜测为鉴权失败。系统不自动探测能力、不改预算、
不换模型/格式、不拆文件；若有效预算确实容纳不下完整 bundle，保持
`model.output_truncated`，由用户提高上限或更换模型。详细设计见
[`模型输出预算显式配置方案`](./2026-08-17-模型输出预算显式配置方案.md)。
...
1. [已完成] 构建并安装绑定最终实现提交的 ML candidate，完成 release、hash 和安装身份核对。
2. [已完成] 为当前 candidate 创建全新物理 Truth、Project、Draft、11 Items、26/26 bindings、
   37/37 manifests、11/11 Resource Prepare Runs 和唯一 Generate Graph。
3. [当前暂停] Graph 在 `item.000.single` 为 1/23；受控复现确认代理接受 4096、拒绝 8192/16384，
   现有 `model.authentication` 是 403 误分类。
4. 实现显式输出预算与 401/403 分类，完成机器门禁；代码变化后构建并安装 replacement candidate，
   再创建全新物理验收根。
5. 证明真实 multi-Item failure 可自动修复并使 parent Run、Graph 和 whole closure succeeded。
6. 核对 ArtifactManifest/hash、ZIP、最终目录、零残留和工程锁恢复。
7. 由用户在真实 STS2 中完成角色、战斗、奖励、选卡、Continue、存档恢复和单 Item 调整验收。

完整 checklist、当前 candidate hashes 和 Run/Graph 诊断边界见本地 Trellis 任务的
`acceptance.md`。当前 installed acceptance 已启动并安全暂停；replacement candidate 的构建、
安装仍需独立授权，真实游戏 UI 只由用户操作。
