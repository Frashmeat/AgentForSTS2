...
Game Pack Stage 2 Work Order 1-7 已落地：目标 crate/DAG、Run/Artifact v3、Pack/Contribution、Truth/Evidence、Resource Workspace、Model/Media ports、9 个 Feature、Shell 切换和旧 `ats-core` 删除已经形成一条生产路径。当前只剩 Work Order 8 的全量独立门禁、Stage 2 新候选及最终人工验收。
...
该方案消除 Mod 领域 Prompt 的代码硬编码：任务自然语言在 Feature Recipe，游戏指导在 Pack，当前事实在 Truth，资源在 Workspace，用户偏好在 `llm.custom_prompt`。代码保留的 role、schema、slot、JSON、枚举、安全、截断和脱敏是协议合同，不放入 Game Pack。
