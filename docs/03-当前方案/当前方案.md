...
可恢复生成主体和当前机器门禁已经完成。安装态 Provider closure 曾暴露 definition-bound Plan 的事实漂移：模型 Plan 与已确认 Character definition 在 HP、Starter Relic 和 Card 集合上冲突。该合同已经修复，绑定 `bb96a80f9de6` 的新 candidate 已通过 release 10/10 和独立完整性复核；O8 现在等待独立安装授权、fresh Provider closure 和用户负责的真实 STS2 单人验收。
...
flowchart TB
    ...
    subgraph Workspace[Project Workspace]
        ...
        Graphs[ExecutionGraphRecord v2]
...
HTTP retry 只处理 transport 层面的有界重试。Decode 或 typed output validation 失败后，只能由用户显式 Resume 产生新 logical attempt。唯一自动语义闭环是 registered-validation-guided repair：用户在初始请求中固定 `until_passed` 或 `max_rounds(n)`，Feature 仅将可唯一归因的 generated-content typed issues 与当前完整 role 文件发送给模型；每轮完整替换后重跑全部 validator。运行中不换策略、模型、endpoint 或 response format。
...
`rc-20260811T023832Z-51e0ae6b61dc` 已安装并形成真实失败证据，但它绑定旧代码，不能继续代表当前修复。新 candidate `rc-20260811T093826Z-bb96a80f9de6` 绑定 `bb96a80f9de6`，已通过 release 10/10、2/2 runtime BuildInfo 和 manifest/installer 独立复算。剩余顺序：

1. 修复提交 `235696d6` 的完整机器门禁和隔离 E2E 已通过。
2. 绑定 `bb96a80f9de6` 的新 candidate 已构建并验证。
3. 取得明确安装授权，安装该 ML candidate 并核对运行时 BuildInfo、variant、features 和 catalog。
4. 创建全新物理 Truth/Project/Draft/Run/Item/Artifact/Package 证据。
5. 完成安装态 Character Plan、Resource binding 和 whole-closure Generate。
6. 核对 Run/Graph/Artifact/hash、最终目录、零残留和工程锁恢复。
7. 由用户在真实 STS2 中完成角色选择、初始状态、一场战斗、自定义奖励、选卡和 Continue/存档恢复。
8. 检查本轮 Mod 日志后，由用户明确确认人工结果，再完成 PRD 与归档。
