...
Provider foundation 已通过 workspace 252/252、Game Context 18/18、STS2 Provider 2/2、desktop composition 10/10、Character 2/2、frontend 38/38、isolated GUI E2E 8/8 及完整静态/构建门禁。GUI 暴露的 frontend request v5/backend v6 envelope 漂移已按破坏性合同同步修复，没有加入兼容 reader 或放宽 Pack/Draft identity。

新 candidate `rc-20260815T133556Z-dbe47d958794` 已绑定实现提交 `dbe47d95`，完成 release 10/10、baseline/ML 双变体验证、ML 安装和已安装 BuildInfo 核对。所有更早 candidate 和失败证据只保留诊断价值；当前 O9 尚未创建 fresh acceptance root 或启动 installed Provider closure。
...
5. O8 及后续 Provider foundation 回归保持通过；当前最终工作树为 workspace 252/252、frontend 38/38、DAG、12-Feature catalog 和 isolated GUI E2E 8/8。

### O9 - 新候选与真实验收

1. [已完成] 构建并安装绑定 `dbe47d95` 的新 ML candidate，release 10/10 且安装身份精确匹配。
...
完整 checklist、最新 Run/Graph 身份和失败证据见本地 Trellis 任务的 `acceptance.md`。candidate 构建和安装已完成；installed acceptance 启动仍保持独立授权，真实游戏 UI 只由用户操作。
