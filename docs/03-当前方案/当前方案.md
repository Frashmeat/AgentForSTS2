...
可恢复生成与 validation-guided repair 主体已提交。绑定 `b68c33df` 的 candidate 已通过 release 10/10、安装和运行时身份核对，但随后 GUI E2E 暴露旧 ExecutionGraph 路径和 project-local 工具输入回退缺陷；当前最终树已修复并通过 225/225 workspace tests、完整机器门禁和隔离 GUI E2E 8/8，因此该 candidate 已失效。O8 仍需构建、安装绑定当前 fallback 提交的新 candidate，执行 fresh Provider closure，并由用户完成真实 STS2 单人验收。
