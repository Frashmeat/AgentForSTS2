...
本轮允许破坏性合同升级，不保留会形成双重事实来源的临时兼容层。Order 1-4 已完成 Pack/Item/readiness、版本化 Item Library、通用编辑器、Resource v2 后端地基、Resource Workbench 和 Relic 纵向 Gate；当前进入 Order 5 Card。每个 Order 完成受影响机器门禁后自动提交；最终集中执行新候选与人工安装验收。
...
- Order 5（下一步）：在同一地基上增加 Card 的 Pack/Truth/Recipe 内容和纵向测试。
- Order 6-7：依次增加 Potion 和 Power。
