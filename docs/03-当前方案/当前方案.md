...
O8 的真实 Provider 验证先证明单次整图 Character Plan 会形成约 20 分钟长连接；即使使用共享 FIFO、显式 `json_object` 和 300/600 秒退避，整个 11-node 响应仍可能因传输失败全部作废。Plan 分阶段恢复落地后，新 candidate 又证明 whole-closure Generate 的 22 个模型阶段存在同类问题：第一次在 Character Plan 截断，第二次 Plan 成功后 Single typed output invalid，而旧 Generate 会在重试时从第一个节点开始。用户于 2026-08-10 确认采用跨游戏的“Runtime 通用执行图 + Feature 分阶段编排 + Pack 声明游戏差异 + Adapter 可靠传输/持久化”方案，并将同一 checkpoint/Resume 合同扩展到 `composition.generate`；当前实现、完整机器门禁和隔离 GUI E2E 8/8 已通过。详细合同、恢复语义和实现状态见 [`可恢复分阶段 Mod 生成架构方案`](./2026-08-10-可恢复分阶段Mod生成架构方案.md)。代码变化已使 `rc-20260810T093305Z-cd38919f727e` 失效，新 candidate 仍待独立授权构建。

独立架构审查随后确认总体分层成立，并要求补齐 Draft/Graph/Run durable commit、active Run claim、禁止 Feature 自动语义重试、严格 Pack v2 node group wire、结构/业务两层恢复、checkpoint 脱敏投影以及 Pause/Cancel/Shutdown 竞态。两轮复核后的合同已经同步到 Runtime、Adapter、Feature、Pack、Tauri、React 和 stable spec；完整 canonical Draft commit payload、排除执行元数据的 validated-content digest、roll-forward-only `commit_prepared`/稳定 `commit_blocked` 均有实现。剩余工作是扩大机器门禁、验证尚未覆盖的竞态/环境路径，再构建绑定新提交的 candidate。
...
- Order 8（进行中）：历史 candidate `rc-20260810T093305Z-cd38919f727e` 保留 whole-closure 失败证据，但已被当前 Generate v2 代码作废。当前工作树已实现 23-node Plan/Single/finalize graph、checkpoint restore、ProjectSession 恢复、通用 Tauri/React controls 和原子 whole-closure publication。最终 GUI gate 又修复了 Plan profile-role 并集与 definition-selected 精确角色集合的错误相等比较。当前 workspace 205 tests、workspace check/clippy、desktop feature checks、TypeScript/frontend/build、DAG、CLI 12 Features、release/build-plan script tests、fmt/diff、真实环境 Character tests 和隔离 GUI E2E 8/8 全部通过；新 candidate、全新安装态 acceptance evidence 和真实 STS2 单人验收仍待执行。

O8 的稳定性修复阶段已经完成代码、本地机器门禁和隔离 GUI E2E：Plan 和 Generate 都使用 Pack-driven execution graph；Generate 将每个 Item 拆成 Plan/Single checkpoint，最后本地恢复全部 proposal、统一 Build/Package 和原子 Artifact 发布。保留证据证明 Pause/Resume/Cancel、invalid-node 精确恢复、23/23 graph、11-Item closure、Run/Artifact/hash、零 transaction/staging 残留和工程锁恢复。下一步是在明确授权后构建绑定当前提交的新 candidate，然后使用全新 Truth/Project/Draft/Run/Item/Artifact/Package 进入安装态 Provider 与真实游戏验收。
