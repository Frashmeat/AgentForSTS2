...
- Order 8（进行中）：`rc-20260808T185834Z-772a4491d5d7` 已通过 10/10 release steps、安装身份和隔离 Truth/工程前置验证；四次真实 `qwen3.7-plus` Plan 均约 418 秒后 `model.transport_failed`，即使外层严格串行并把额外冷却提高到 300 秒和 600 秒也未成功。根因是外层间隔不能约束一个 Plan 内部以 10/30 秒 fallback 发出的三次 HTTP attempts。当前修复让 composition root 持有唯一 FIFO 模型队列，一个逻辑任务连同重试占有席位直到终态，并把无 Provider 指引的退避改为兼容旧配置的可配置 120/300 秒默认值；修复后的完整机器门禁已通过。该代码变化使 candidate 失效；新 candidate、成功 Draft/Item/Artifact/Package 和全新存档的 STS2 单人可玩闭环仍待完成。
...
- `e00f0fa8` 候选只证明上一轮 Pack v3 / ItemDefinition v1；`rc-20260806T161945Z-366203d42b3e`、`rc-20260807T075958Z-551f1df51fe2` 和 `rc-20260808T153706Z-a0ea644832f7` 均保留为对应缺陷/上游失败证据，不能用于修复后的最终验收。O8 最终验收必须安装绑定轮询修复提交的新 candidate，并使用新的 Item graph、Run/Artifact/Package 和新游戏存档，不得复用或重命名旧结果。
