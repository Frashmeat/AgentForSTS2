...
- Order 8（进行中）：修复后 candidate `rc-20260809T152710Z-9d0e390a513a` 已绑定 clean commit `9d0e390a513aa0ff4445b53631911dd3466e586b`，通过 10/10 release steps、baseline/ML manifest/hash、runtime BuildInfo 和零 candidate residue；ML 安装态 UI 精确报告本 Build ID、`variant=ml` 和 12 Features。全新隔离 app-data 已从 verified Stage 1 输入重新导入 Pack v4 Truth，新工程 `project.create` succeeded。`deepseek-v4-flash/json_object` 的精确 Prototype Character Plan 在同一 FIFO Run 内按 300/600 秒退避耗尽后以上游 `model.transport_failed` 终止，未再现格式/解码错误，也未创建 Draft/Item/Artifact/Package。退出后工程锁恢复，stdout/stderr 为空且零 staging/transaction/journal 残留。因此本地代码/格式兼容前置已验证，当前阻塞仍是 Provider 长输出传输稳定性；Character 闭包生成与真实 STS2 单人验收仍待完成。
