...
function Resolve-LauncherLayout {
    ...
    $rootCandidate = (Resolve-Path (Join-Path $scriptDir "..")).Path

    $repoBackend = Join-Path $rootCandidate "backend\main_workstation.py"
    if (Test-Path -LiteralPath $repoBackend) {
        return @{
            Mode = "repo"
            Root = $rootCandidate
            BackendRoot = Join-Path $rootCandidate "backend"
            FrontendRoot = Join-Path $rootCandidate "frontend"
            FrontendDist = Join-Path $rootCandidate "frontend\dist"
            ConfigPath = Join-Path $rootCandidate "config.json"
            ConfigExamplePath = Join-Path $rootCandidate "config.example.json"
            RuntimeDir = Join-Path $rootCandidate "runtime"
        }
    }

    $releaseBackend = Join-Path $rootCandidate "services\workstation\backend\main_workstation.py"
    if (Test-Path -LiteralPath $releaseBackend) {
        $serviceRoot = Join-Path $rootCandidate "services\workstation"
        return @{
            Mode = "release"
            Root = $rootCandidate
