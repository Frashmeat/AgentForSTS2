...
def test_me_router_returns_409_when_server_workspace_is_busy(client: TestClient):
    ...
    detail = client.get(f"/api/me/jobs/{created.json()['id']}")
    listed = client.get("/api/me/jobs")
    events = client.get(f"/api/me/jobs/{created.json()['id']}/events")
...
def test_me_router_can_complete_batch_card_fullscreen_with_uploaded_asset(client: TestClient):
    registered = client.post(
        "/api/auth/register",
        json={
            "username": "luna",
