...
from app.modules.platform.infra.persistence.repositories.quota_balance_repository_sqlalchemy import (
...
def test_quota_billing_service_reserves_and_refunds_execution_usage(db_session):
    ...
    balance_repository = QuotaBalanceRepositorySqlAlchemy(db_session)
    service = QuotaBillingService(
        ...
        quota_balance_repository=balance_repository,
    )
    now = datetime.now(UTC)
    account = quota_repository.create_account(QuotaAccountRecord(user_id=1001, status=QuotaAccountStatus.ACTIVE))
    balance_repository.create_balance(
        QuotaBalanceRecord(user_id=1001, quota_account_id=account.id, total_limit=10, status=QuotaAccountStatus.ACTIVE)
    )
...
def test_quota_billing_service_does_not_double_refund_same_execution(db_session):
    ...
    now = datetime.now(UTC)
    account = quota_repository.create_account(QuotaAccountRecord(user_id=1001, status=QuotaAccountStatus.ACTIVE))
    balance = balance_repository.create_balance(
        QuotaBalanceRecord(user_id=1001, quota_account_id=account.id, total_limit=10, status=QuotaAccountStatus.ACTIVE)
    )
    ...
    assert balance.refunded_amount == 1
...
def test_quota_billing_service_reports_quota_exhausted_when_no_available_bucket(db_session):
    service = QuotaBillingService(
        ...
        usage_ledger_repository=UsageLedgerRepositorySqlAlchemy(db_session),
        quota_balance_repository=QuotaBalanceRepositorySqlAlchemy(db_session),
    )
