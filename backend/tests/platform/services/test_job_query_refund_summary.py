from __future__ import annotations

import sys
from datetime import UTC, datetime, timedelta
from pathlib import Path

sys.path.insert(0, str(Path(__file__).resolve().parents[3]))

from app.modules.platform.application.services.job_query_service import JobQueryService
from app.modules.platform.contracts.job_commands import CreateJobCommand
from app.modules.platform.infra.persistence.models import (
    AIExecutionRecord,
    ChargeStatus,
    ExecutionChargeRecord,
    QuotaAccountRecord,
    QuotaBucketRecord,
)
from app.modules.platform.infra.persistence.repositories.job_query_repository_sqlalchemy import JobQueryRepositorySqlAlchemy
from app.modules.platform.infra.persistence.repositories.job_repository_sqlalchemy import JobRepositorySqlAlchemy
from app.modules.platform.infra.persistence.repositories.quota_query_repository_sqlalchemy import (
    QuotaQueryRepositorySqlAlchemy,
)


def test_job_query_service_exposes_refund_summary_to_user_views(db_session):
    job_repository = JobRepositorySqlAlchemy(db_session)
    job = job_repository.create_job_with_items(
        user_id=1001,
        command=CreateJobCommand.model_validate(
            {"job_type": "single_generate", "workflow_version": "2026.04.03", "items": [{"item_type": "card"}]}
        ),
    )
    db_session.flush()
    execution = AIExecutionRecord(
        job_id=job.id,
        job_item_id=job.items[0].id,
        user_id=1001,
        status="completed_with_refund",
        provider="openai",
        model="gpt-5.4",
        request_idempotency_key="idem-refund-service",
        workflow_version="2026.04.03",
        step_protocol_version="v1",
        result_schema_version="v1",
        step_type="image.generate",
        step_id="step-1",
    )
    db_session.add(execution)
    db_session.flush()
    db_session.add(
        ExecutionChargeRecord(
            ai_execution_id=execution.id,
            user_id=1001,
            charge_status=ChargeStatus.REFUNDED,
            charge_amount=1,
            refund_reason="cancelled_after_start",
        )
    )
    quota_account = QuotaAccountRecord(user_id=1001, status="active")
    db_session.add(quota_account)
    db_session.flush()
    db_session.add(
        QuotaBucketRecord(
            quota_account_id=quota_account.id,
            bucket_type="daily",
            period_start=datetime.now(UTC) - timedelta(hours=1),
            period_end=datetime.now(UTC) + timedelta(hours=23),
            quota_limit=10,
            used_amount=2,
            refunded_amount=1,
        )
    )
    db_session.commit()

    service = JobQueryService(
        job_query_repository=JobQueryRepositorySqlAlchemy(db_session),
        quota_query_repository=QuotaQueryRepositorySqlAlchemy(db_session),
    )

    jobs = service.list_jobs(1001)
    detail = service.get_job_detail(1001, job.id)

    assert jobs[0].original_deducted == 1
    assert jobs[0].refunded_amount == 1
    assert jobs[0].net_consumed == 0
    assert jobs[0].refund_reason_summary == "cancelled_after_start"
    assert detail is not None
    assert detail.original_deducted == 1
    assert detail.refunded_amount == 1
    assert detail.net_consumed == 0
    assert detail.refund_reason_summary == "cancelled_after_start"
