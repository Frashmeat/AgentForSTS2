...
11. Localization JSON must be **flat key-value**, NOT nested objects. `{"FOO.title": "Bar"}` ✅  `{"FOO": {"title": "Bar"}}` ❌ → `LocException: token type 'StartObject' as a string` → game crashes on startup.
    Localization rich text may use only the exact tags declared by the active Game Pack, with balanced open/close pairs. Use `[blue]1[/blue]` for highlighted numeric values. Never invent tags such as `[yellow]`, use attribute forms such as `[color=yellow]`, or put raw ASCII square-bracket text in a localization value; the renderer may display it literally or fail to parse it.
