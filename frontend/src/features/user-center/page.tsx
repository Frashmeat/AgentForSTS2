...
export function UserCenterPage() {
  ...
  if (isLoading) {
    return (
      <PlatformPageShell kicker="User Center" title="用户中心" description="正在恢复会话并同步平台账号状态。" actions={homeAction}>
  ...
  if (!isAuthenticated) {
    return (
      <PlatformPageShell kicker="User Center" title="用户中心" description="登录后查看平台任务、统一次数池和返还记录。" actions={homeAction}>
        <section className="platform-page-card p-8">
          ...
          <Link
            to="/auth/login"
            className="platform-page-primary-button mt-6 inline-flex"
          >
            去登录
          </Link>
        </section>
  ...
  if (error) {
    return (
      <PlatformPageShell kicker="User Center" title="用户中心" description="平台数据加载失败，请检查登录态或后端配置。" actions={homeAction}>
  ...
  if (overview === null) {
    return (
      <PlatformPageShell kicker="User Center" title="用户中心" description="正在读取平台任务与配额信息。" actions={homeAction}>
