        ...
        <div className="rounded-lg border border-white bg-white/85 p-4 shadow-sm">
          <div className="mb-4 flex items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-xs text-slate-500">{form.id === null ? "新增执行配置" : `正在编辑 #${form.id}`}</p>
            </div>
          ...
          <div className="grid gap-3">
            <label className="space-y-1 text-sm text-slate-600">
              <span>编号</span>
              <input value={form.code} onChange={(event) => patchForm({ code: event.target.value })} className="w-full rounded-lg border border-slate-200 px-3 py-2" placeholder="codex-gpt-5-5" />
            </label>
            <label className="space-y-1 text-sm text-slate-600">
              <span>名称</span>
              <input value={form.display_name} onChange={(event) => patchForm({ display_name: event.target.value })} className="w-full rounded-lg border border-slate-200 px-3 py-2" placeholder="Codex CLI / gpt-5.5" />
            </label>
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-1 2xl:grid-cols-2">
              <label className="space-y-1 text-sm text-slate-600">
                <span>Agent 后端</span>
                <select value={form.agent_backend} onChange={(event) => patchForm({ agent_backend: event.target.value === "claude" ? "claude" : "codex" })} className="w-full rounded-lg border border-slate-200 px-3 py-2">
              ...
              <label className="space-y-1 text-sm text-slate-600">
                <span>模型</span>
                <input value={form.model} onChange={(event) => patchForm({ model: event.target.value })} className="w-full rounded-lg border border-slate-200 px-3 py-2" placeholder="gpt-5.5" />
              </label>
            </div>
            <label className="space-y-1 text-sm text-slate-600">
              <span>描述</span>
              <textarea value={form.description} onChange={(event) => patchForm({ description: event.target.value })} className="min-h-20 w-full rounded-lg border border-slate-200 px-3 py-2" />
            </label>
            <label className="space-y-1 text-sm text-slate-600">
              <span>排序</span>
              <input type="number" value={form.sort_order} onChange={(event) => patchForm({ sort_order: Number(event.target.value) || 0 })} className="w-full rounded-lg border border-slate-200 px-3 py-2" />
            </label>
          </div>
          <div className="mt-3 flex flex-wrap gap-4">
            ...
            <label className="inline-flex items-center gap-2 text-sm text-slate-600">
              <input type="checkbox" checked={form.recommended} onChange={(event) => patchForm({ recommended: event.target.checked })} />
              <span>推荐配置</span>
            </label>
          </div>
          <div className="mt-4 flex flex-wrap items-center gap-2">
            <button type="button" onClick={() => void submitForm()} disabled={saving || !form.code || !form.display_name || !form.model} className="inline-flex items-center gap-2 rounded-lg bg-violet-700 px-3 py-2 text-sm font-medium text-white transition hover:bg-violet-800 disabled:opacity-50">
              <Save size={16} />
                    ...
                    <tr key={profile.id}>
                      ...
                      <td className="px-3 py-2">
                        <div className="flex flex-wrap gap-2">
                          <button type="button" onClick={() => startEdit(profile)} className="rounded-lg border border-slate-200 px-2.5 py-1.5 text-xs text-slate-600 transition hover:border-violet-200 hover:text-violet-700">编辑</button>
                          {profile.enabled ? (
                            <button type="button" onClick={() => void runProfileAction(() => disableAdminExecutionProfile(profile.id), "执行配置已禁用。")} className="rounded-lg border border-slate-200 px-2.5 py-1.5 text-xs text-slate-600 transition hover:border-violet-200 hover:text-violet-700" disabled={saving}>禁用</button>
                          ) : (
                            <button type="button" onClick={() => void runProfileAction(() => enableAdminExecutionProfile(profile.id), "执行配置已启用。")} className="rounded-lg border border-slate-200 px-2.5 py-1.5 text-xs text-slate-600 transition hover:border-violet-200 hover:text-violet-700" disabled={saving}>启用</button>
                          )}
                          <button type="button" onClick={() => void runProfileAction(() => deleteAdminExecutionProfile(profile.id), "执行配置已删除。")} className="inline-flex items-center gap-1 rounded-lg border border-rose-100 px-2.5 py-1.5 text-xs text-rose-600 transition hover:border-rose-200 hover:bg-rose-50" disabled={saving}>
                            <Trash2 size={13} />
                            <span>删除</span>
                          </button>
                        </div>
