...
test("createBatchItemStateRecord initializes all items with default state", () => {
  const record = createBatchItemStateRecord([
    { id: "item-1" },
    { id: "item-2" },
  ]);
...
test("updateBatchItemStateRecord merges patch onto target item", () => {
  const next = updateBatchItemStateRecord(
    { "item-1": createDefaultBatchItemState() },
    "item-1",
    { status: "done", currentPrompt: "prompt" },
  );
...
test("append helpers preserve history and logs", () => {
  let next = appendBatchItemProgress(
    { "item-1": createDefaultBatchItemState() },
    "item-1",
    "planning",
  );
...
test("applyBatchItemImage writes image by index and enters awaiting selection", () => {
  const next = applyBatchItemImage(
    { "item-1": createDefaultBatchItemState() },
    "item-1",
    "img-b64",
    0,
