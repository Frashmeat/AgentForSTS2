...
| 独立前端态 | `tools\start_split_local.bat` 或独立静态站点 | 独立静态前端 | `runtime-config.js` 指向本机或 LAN 可达 `workstation-backend` | `runtime-config.js` 指向 `web-backend` | 前端独立发布、平台接口与工作台接口分离 |
...
- 若用户侧发放安装包，推荐内容为“独立静态前端 + workstation-backend + launcher”，`web-backend` 继续单独部署
