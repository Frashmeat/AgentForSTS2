...
| 正式部署目标 `hybrid` | `tools\latest\package-release.ps1 hybrid` | 独立静态前端 | `runtime-config.js` 指向本机或 LAN 可达 `workstation-backend` | `runtime-config.js` 指向独立部署的 `web-backend` | 用户侧正式交付、“一个前端入口 + 两类后端能力” |
...
- `web-backend` 继续单独部署，不打进 `hybrid` 用户包
